...
- `docs/api/api-contract-source-of-truth.md`：接口契约真源。
- `docs/api/resource-model.md`：资源、领域对象、DTO、数据库对象的映射关系。
- `docs/api/openapi.yaml` 或 `docs/api/openapi.md`：OpenAPI 草案。
- `docs/api/error-response-and-codes.md`：HTTP 状态码、业务错误码、错误响应结构。
- `docs/api/auth-and-permission-contract.md`：认证、授权、角色与权限检查点。
- `docs/api/pagination-filtering-sorting.md`：分页、过滤、排序、搜索约定。
- `docs/api/mock-and-contract-testing-plan.md`：Mock、契约测试和前后端联调计划。
- `docs/api/integration-risk-list.md`：跨端字段、时区、金额、幂等、并发、第三方依赖风险。
