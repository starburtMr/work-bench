...
ESLint serves as a valuable linting tool for JavaScript and TypeScript, helping developers maintain code quality and adhere to coding standards. This skeleton uses `.eslintrc.cjs` to enforce import direction, file naming, React, accessibility, testing, and formatting rules.

[ESLint Configuration Example Code](../.eslintrc.cjs)

#### Prettier

Prettier maintains consistent formatting according to `.prettierrc`. Keep format-on-save enabled in the IDE when adapting the skeleton so generated and hand-written code stay consistent.
