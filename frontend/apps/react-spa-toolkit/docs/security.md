...
User info should be considered application-level state that is available wherever route protection or UI personalization needs it. This skeleton uses `react-query-auth` with a small local wrapper in `src/lib/auth.tsx`; a copied project can keep that wrapper or replace it with context, hooks, or another state-management library.
...
[RBAC Example Code](../src/features/discussions/components/delete-discussion.tsx)
