# Next.js App Router Toolkit

Reusable Next.js App Router skeleton for authenticated, API-backed product frontends. It is a work-bench frontend candidate, not a finished product.

## Best Fit

Use this skeleton when the target project needs:

- Next.js App Router layouts, route groups, and nested routes;
- server components by default with client components for interaction;
...
Use another skeleton when the project needs legacy Pages Router maintenance, a pure client-side SPA, or a backend-only scaffold.
...
Treat this domain as a replaceable reference flow. It exists to show App Router composition, auth, server-state, forms, feature boundaries, mocks, tests, and e2e coverage in a realistic shape.
...
```text
src/
  app/          App Router layouts, pages, route groups, and providers
  components/   shared UI, layouts, errors, and primitives
...
Keep route files thin. Put route composition in `src/app`, business behavior inside `src/features`, and reusable UI or adapters in shared folders.
...
Remove generated directories such as `node_modules`, `.next`, `coverage`, `playwright-report`, `test-results`, `storybook-static`, `.env`, and `mocked-db.json` before committing skeleton changes.
